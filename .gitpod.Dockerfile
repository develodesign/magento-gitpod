...
RUN if [ ! "$XDEBUG_DEFAULT_ENABLED" = "YES" ]; then sudo echo "xdebug.remote_enable=on" >> /etc/php/${PHP_VERSION}/mods-available/xdebug.ini \
    ...
    && mv /etc/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini /etc/php/${PHP_VERSION}/cli/conf.d/20-xdebug.ini-bak; \
fi
